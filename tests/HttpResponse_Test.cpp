...
// Global buffer to capture output from lwip_send.
static std::string g_lwip_sent_buffer;


extern "C" ssize_t test_lwip_send(int s, const void *data, size_t size, int flags) {
    (void)s; (void)flags; // These parameters are ignored in our test
    g_lwip_sent_buffer.append(reinterpret_cast<const char*>(data), size);
    return size;
}
...
TEST(HttpResponse, SendMethodSendsHeadersAndBody)
{
    ...
    sprintf(expectedContentLength, "Content-Length: %zu", body.size());
