 ...
 * @author Ian Archbell
 * @brief Implementation of JwtAuthenticator for JWT creation and validation.
 *
...
/// @copydoc JwtAuthenticator::JwtAuthenticator
JwtAuthenticator::JwtAuthenticator()
{
    secretKey = JWT_SECRET;
}

/// @copydoc JwtAuthenticator::base64urlEncode
...
/// @copydoc JwtAuthenticator::base64urlDecode
bool JwtAuthenticator::base64urlDecode(const std::string &input, std::string &output) const
{
...
/// @copydoc JwtAuthenticator::isBase64urlEncoded
...
/// @copydoc JwtAuthenticator::hmacSHA256
...
/// @copydoc JwtAuthenticator::generateJWT
std::string JwtAuthenticator::generateJWT(const std::string &userId, const std::string &userName) const
{
...
/// @copydoc JwtAuthenticator::decodeJWT
bool JwtAuthenticator::decodeJWT(const std::string &token, std::string &header, std::string &payload, std::string &signature) const
{
...
/// @copydoc JwtAuthenticator::bytesToBase64url
std::string JwtAuthenticator::bytesToBase64url(const unsigned char *data, size_t length) const
{
...
/// @copydoc JwtAuthenticator::verifyJWTSignature
bool JwtAuthenticator::verifyJWTSignature(const std::string &encoded_header, const std::string &encoded_payload, const std::string &signature) const
{
...
/// @copydoc JwtAuthenticator::isJWTPayloadExpired
...
/// @copydoc JwtAuthenticator::isJWTExpired
...
/// @copydoc JwtAuthenticator::validateJWT
bool JwtAuthenticator::validateJWT(const std::string &token, bool validateExpiry) const
{
    size_t first_dot = token.find('.');
    size_t second_dot = token.find('.', first_dot + 1);
    if (first_dot == std::string::npos || second_dot == std::string::npos)
...
void JwtAuthenticator::init(const std::string &secret, int expirySeconds)
{
