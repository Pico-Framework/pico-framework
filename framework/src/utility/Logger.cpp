 ...
 */
 ...
 void Logger::enableFileLogging(FatFsStorageManager* sm, const std::string& path) {
     fileLogger = sm;
     logPath = path;
     logToFile = (fileLogger != nullptr && !logPath.empty());
 }
 ...
 void Logger::log(LogLevel level, const char* msg) {
     ...
     if (logToFile && fileLogger) {
         char fullMsg[256];
         snprintf(fullMsg, sizeof(fullMsg), "[%s] [%s] %s\n", timeBuf, levelStr, msg);
         fileLogger->appendToFile(logPath, reinterpret_cast<const uint8_t*>(fullMsg), strlen(fullMsg));
     }
 }
